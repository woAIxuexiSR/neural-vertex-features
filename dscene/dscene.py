...
class DynamicScene:
    ...
    # helper function for getting the var index of a shape
    def get_shape_var_idx(self, shape_name):
        for idx, (param_name, _, _) in self.common_vars.items():
            if shape_name == param_name.split('.')[0]:
                return idx
        for idx, (shape_names, _, _) in self.translate_vars.items():
            if shape_name in shape_names:
                return idx
        for idx, (shape_names, _, _, _, _) in self.rotate_vars.items():
            if shape_name in shape_names:
                return idx
        for idx, (shape_names, _, _) in self.scale_vars.items():
            if shape_name in shape_names:
                return idx
        return -1
