...
class DynamicVModel(nn.Module):
    def __init__(self, dscene, config: dict):
        super().__init__()

        self.feature_dim = config["feature_dim"]
        self.time_steps = config["time_steps"]
        self.vfeature_dim = self.feature_dim * self.time_steps
        self.n_hidden_layers = config["n_hidden_layers"]
        self.n_neurons = config["n_neurons"]

        self.vertex_feature = VertexFeature(dscene.scene, self.vfeature_dim)

        # encoding
        self.encoding = tcnn.Encoding(3 + 3 + 3, encoding_config)
        
        # mlp
        self.n_input_dims = self.vfeature_dim + self.encoding.n_output_dims
        layers = []
    ...
    def forward(self, si, vars: np.ndarray):
        ...
        t = vars[0] * self.time_steps
        ti, tf = int(t), t - int(t)
        xl = x[:, ti * self.feature_dim : (ti + 1) * self.feature_dim]
        xh = x[:, (ti + 1) * self.feature_dim : (ti + 2) * self.feature_dim]
        x = xl * (1 - tf) + xh * tf
