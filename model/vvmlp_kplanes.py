...
class VVMlpKPlanes(nn.Module):
    ...
    def __init__(self, dscene, config: dict):
        ...
        self.n_input_dims = 3 + 3 * self.var_num * 2 + 3 + 3 + 3
    ...
    def forward(self, si, vars: np.ndarray):
        ...
        # torch.cuda.synchronize()
        # end = time.time()
        # print("prepare_input time: ", end - start)
        # start = time.time()
        ...
        # torch.cuda.synchronize()
        # end = time.time()
        # print("encoding x time: ", end - start)
        # start = time.time()
