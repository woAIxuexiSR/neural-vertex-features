...
def weighted_l2_loss(lhs, rhs, weight):
    rhs = rhs.mean(dim=1)
    return ((lhs - rhs) ** 2) / weight
...
def normed_semi_l2_loss(lhs, rhs, weight):
    rhs = rhs.mean(dim=1).detach()
    norm = (lhs + rhs).detach() / 2 + EPS
    return torch.mean(((lhs / norm) - (rhs / norm)) ** 2)

def weighted_normed_semi_l2_loss(lhs, rhs, weight):
    rhs = rhs.mean(dim=1).detach()
    norm = (lhs + rhs).detach() / 2 + EPS
    return (((lhs / norm) - (rhs / norm)) ** 2) / weight
...
def normed_semi_l1_loss(lhs, rhs):
    rhs = rhs.mean(dim=1).detach()
    norm = (lhs + rhs).detach() / 2 + EPS
    return torch.mean(torch.abs((lhs / norm) - (rhs / norm)))
